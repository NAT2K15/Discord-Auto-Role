...
nat2k15.on('ready', () => {
    console.log(`I have connected to ${nat2k15.user.tag} and watching ${nat2k15.guilds.cache.size} servers.`)
})


nat2k15.on('messageCreate', async(message) => {
        ...
        if (message.member.permissions.has(config.perms)) {
            ...
            if (!args[0]) {
                ...
                let e2 = new MessageEmbed()
                    .setDescription(`Please input the msg. \`${config.command} <role> <message>\``)
                    .setColor(config.embed.color)
                    .setFooter(config.embed.footer)
                ...
                let row = new MessageActionRow()
                    .addComponents(
                        new MessageButton()
                        .setLabel(`React to Receive: ${role.name}`)
                        .setStyle("PRIMARY")
...
nat2k15.on('interactionCreate', async(interaction) => {
    ...
    if (interaction.member.roles.cache.has(role.id)) {
        ...
        let e1 = new MessageEmbed()
            .setColor(config.embed.color)
            .setDescription(`${role.name} has been removed from you.`)
        interaction.followUp({ embeds: [e1], ephemeral: true })
    } else {
        interaction.member.roles.add(role.id)
        let e1 = new MessageEmbed()
            .setColor(config.embed.color)
            .setDescription(`${role.name} has been added to you.`)
        interaction.followUp({ embeds: [e1], ephemeral: true })
    }
...
nat2k15.login(config.token).catch(e => {
    if (e) {
